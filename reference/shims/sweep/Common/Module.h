...
//-------------------------------------------------------------------------------------------------
// BFME shadow-shim: retail lotrbfme.exe compiles friend_newModuleInstance as
//   ::new cls(thing, moduleData)  (global operator new + placement ctor)
// NOT ZH's pool placement newInstance(cls). Proven by the retail bodies:
//   push sizeof(cls); call ??2@YAPAXI@Z; ...; new(block) cls(...)  (@0x514790).
// The pool glue (getClassMemoryPool) is emitted from module_pool_glue_bulk.cpp.
#define MAKE_STANDARD_MODULE_MACRO( cls ) \
public: \
	static Module* friend_newModuleInstance( Thing *thing, const ModuleData* moduleData ) { return ::new cls( thing, moduleData ); } \

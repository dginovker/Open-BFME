...
private:
  ...
  Bool              m_oldFaction;                       ///< Faction existed in the original Generals
	...
	// BFME-LAYOUT-PATCH: retail sizeof(PlayerTemplate) is 0x124, not ZH's 0xf0 (proven by the
	// stride literal in PlayerTemplateStore::findPlayerTemplate @0x4E0F30: add eax,0x124).
	// BFME added ~13 dwords of fields; until they're identified, tail padding reproduces the
	// layout every stride-user compiles against. Field names TBD via future member-access RE.
	char m_bfmeLayoutPad[0x34];
};

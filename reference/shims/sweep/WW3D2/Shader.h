// Shader.h -- BFME shadow shim. Verbatim ZH GeneralsMD WW3D2 shader.h except:
//  * retail lotrbfme.exe's ShaderClass is 16 bytes, not 4: W3DStatusCircle
//    members after m_shaderClass sit at +0xc vs the ZH layout (retail
//    m_vertexBufferCircle at this+0xe4, m_vertexBufferScreen at this+0xe8;
//    ZH gives 0xd8/0xdc). BFME added three extra dwords after ShaderBits
//    (names/purpose pending RE; the packed render-state word ShaderBits
//    stays at offset 0 since retail inlines like Set_Depth_Mask still
//    address [shader+0]).
// Injected into every swept TU via the PreRTS.h shim so the whole engine
// sees the retail 16-byte ShaderClass consistently.
/*
...
protected:
	...
	// BFME: three extra dwords present in retail (see header comment).
	unsigned int bfme_reserved_4;
	unsigned int bfme_reserved_8;
	unsigned int bfme_reserved_C;
